...
void top_left(t_game *game, double ray_counter)
{
	...
	while (1)//east texture
	{
		...
		if (find_wall_vertical_two(game->player.pos_x - hor, game->player.pos_y - y, game))
		{
			...
			else if (find_wall_vertical_two(game->player.pos_x - hor, game->player.pos_y - y, game) == 3) // door
			{
	...
	{
		if (find_wall_horizontal_two(game->player.pos_x - y, game->player.pos_y - ver, game) == 3) // door
		{
