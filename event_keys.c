...
void	key_m(t_game *game)
{
	...
	else
	{
		...
		game->minimap.shift_x = ((SCREEN_WID - game->minimap.full_scale * \
			(game->map.total_column - game->minimap.empty_column - \
			game->minimap.empty_column2)) - game->minimap.full_scale) / 2;
		game->minimap.shift_y = ((SCREEN_LEN - (game->map.length + 1) * \
			game->minimap.full_scale) - game->minimap.full_scale) / 2;
	}
